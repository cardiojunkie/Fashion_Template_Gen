...
- NVIDIA runtime: fixed endpoint/model/request settings, missing/auth/rate-limit/timeout/redirect/
  malformed/duplicate/oversized response handling, exact-model guided-JSON blue-square diagnostic
  with non-forced alternatives, session/key-bound extraction gating, image serialization, safe
  response metadata, job fingerprints, and plaintext-secret absence.
